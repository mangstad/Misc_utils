...
LearnScale = p.Results.LearnScale;
...
for iFold = 1:nFold
    ...
    if (NumComps>1)
        ...
        for iNest = 1:NumComps
            tempresults = mc_bbs(featuremat(train_idx,:),pheno(train_idx,:),nuisance(train_idx,:),nestfold,NumComp(iNest),'Scores',nestAa,'LOSOPheno',LOSOPheno);
        ...
        for iPheno = 1:size(pheno,2)
            ...
            if (isempty(testpheno))
                ...
                pheno_residualized(test_idx,iPheno) = testpheno(test_idx,iPheno) - [ones(n_testf,1) nuisance(test_idx,:)]*b([1 (kk+2):end]);
            end
        ...
        for iPheno = 1:size(pheno,2)
            ...
            if (isempty(testpheno))
                ...
                pheno_residualized(test_idx,iPheno) = testpheno(test_idx,iPheno) - [ones(n_testf,1) nuisance(test_idx,:)]*b([1 (k+2):end]);
            end
            
            if (LearnScale)
                nestAa = [];
                cfolds = mc_correct_folds(folds(train_idx));
                for iNest = 1:max(cfolds)
                    nestAa{iNest} = Aa{iFold}(train_idx,:);
                end
                tempresults = mc_bbs(featuremat(train_idx,:),pheno(train_idx,iPheno),nuisance(train_idx,:),cfolds,NumComp,'Scores',nestAa,'LOSOPheno',LOSOPheno);
                scale = std(tempresults.pheno_residualized)/std(tempresults.pheno_predict);
                pheno_predict(test_idx,iPheno) = pheno_predict(test_idx,iPheno).*scale;
                results.scale(iFold) = scale;
            end
        end
    end
end
